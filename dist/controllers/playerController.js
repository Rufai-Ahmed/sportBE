...
const createPlayer = async (req, res) => {
    try {
        ...
        const player = await player_1.default.create({ username, password, phoneNumber, photo });
...
const loginPlayer = async (req, res) => {
    try {
        ...
        // Generate JWT
        const token = jsonwebtoken_1.default.sign({ id: player._id }, config_1.config.jwtSecret, { expiresIn: '1h' });
        res.json({ token });
    }
    catch (error) {
        res.status(400).json({ message: error.message });
